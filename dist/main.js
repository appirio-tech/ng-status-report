...
angular.module("appirio-tech-ng-status-report").run(["$templateCache", function($templateCache) {$templateCache.put("views/create-status-report.directive.html","<loader ng-show=\"false\"></loader><form><ul><li><label>project status</label></li><li><label>status message</label></li><li><textarea placeholder=\"Status message\" ng-model=\"vm.statusReport.text\" class=\"widest\"></textarea></li><li><label>project links</label></li><li class=\"project-links\"><div class=\"flex middle\"><input type=\"url\" placeholder=\"http://www.example.com\" ng-model=\"vm.currentLink\" class=\"wider\"/><button ng-click=\"vm.addLink()\" class=\"clean plus\"><div class=\"icon plus hollow\"></div></button></div><ul><li ng-repeat=\"link in vm.statusReport.links track by $index\" class=\"flex middle space-between\"><p class=\"link\">{{link.url}}</p><button ng-click=\"vm.removeLink(link)\" class=\"clean\">remove</button></li></ul></li><li><label>project images</label></li><li><div class=\"flex middle\"><input type=\"text\" placeholder=\"Image caption\" ng-model=\"vm.currentCaption\" class=\"wider\"/><button ng-click=\"vm.addCaption()\" class=\"clean plus\"><div class=\"icon plus hollow\"></div></button></div><ul><li ng-repeat=\"caption in vm.captions track by $index\" class=\"flex middle space-between\"><p class=\"caption\">{{caption}}</p><button ng-click=\"vm.removeCaption(caption)\" class=\"clean\">remove</button></li></ul><ap-uploader config=\"vm.uploaderConfig\" uploading=\"vm.uploaderUploading\" has-errors=\"vm.uploaderHasErrors\" has-files=\"vm.uploaderHasFiles\" file-array=\"vm.uploaderFiles\"></ap-uploader></li><li><button ng-click=\"vm.create()\" class=\"action\">post status to timeline</button></li></ul></form>");
...
(function() {
  ...
  CreateController = function($scope, $state, API_URL, StatusReportAPIService) {
    ...
    activate = function() {
      ...
      $scope.$watch('vm.uploaderFiles', function(newFiles) {
        if (newFiles != null ? newFiles.length : void 0) {
          return vm.statusReport.images = newFiles.map(function(file, index) {
            return {
