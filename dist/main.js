...
angular.module("appirio-tech-ng-status-report").run(["$templateCache", function($templateCache) {$templateCache.put("views/create-status-report.directive.html","<loader ng-show=\"false\"></loader><form><ul><li><label>status update</label></li><li><textarea placeholder=\"Please provide a short status update to the customer. Be sure to describe  the progress, mention issues, and respond to any prior customer questions.\" ng-model=\"vm.statusReport.text\" class=\"widest\"></textarea></li><li><label>Links to demos and code</label></li><li class=\"project-links\"><div class=\"flex middle\"><input type=\"url\" placeholder=\"http://www.example.com\" ng-model=\"vm.currentLink\" class=\"wider\"/><button ng-click=\"vm.addLink()\" class=\"clean plus\"><div class=\"icon plus hollow\"></div></button></div><ul><li ng-repeat=\"link in vm.statusReport.links track by $index\" class=\"flex middle space-between\"><p class=\"link\">{{link.url}}</p><button ng-click=\"vm.removeLink(link)\" class=\"clean\">remove</button></li></ul></li><li><label>Screenshots and documents</label></li><li><ap-uploader config=\"vm.uploaderConfig\" uploading=\"vm.uploaderUploading\" has-errors=\"vm.uploaderHasErrors\" has-files=\"vm.uploaderHasFiles\"></ap-uploader></li><li><button ng-click=\"vm.create()\" class=\"action\">send to customer</button></li></ul></form>");
$templateCache.put("views/detail-status-report.directive.html","<loader ng-show=\"false\"></loader><h1>  {{vm.report.createdAt | date:\'MMMM d yyyy\'}} status report</h1><time>{{vm.report.createdAt | timeLapse }}</time><p class=\"message\">{{vm.report.text}}</p><ul class=\"links\"><li ng-repeat=\"link in vm.report.links\"><a href=\"{{link.url}}\">{{link.url}}</a></li></ul><ul class=\"files flex wrap center\"><li ng-repeat=\"image in vm.report.images\"><a href=\"{{image.preSignedURL}}\"><img src=\"{{image.preSignedURL}}\"/></a></li></ul>");
...
$templateCache.put("views/status-reports.directive.html","<div class=\"flex rows\"><nav><h6>previous status reports</h6><past-status-reports work-id=\"{{vm.workId}}\"></past-status-reports></nav><main><h3>create a new status report</h3><hr class=\"biggest\"/><create-status-report work-id=\"{{vm.workId}}\"></create-status-report></main></div>");}]);
(function() {
  'use strict';
  var directive;
    ...
    configureUploader = function(workId, assetType) {
      ...
      uploaderConfig = {
        ...
        onCaptionChange: function(data) {
          return vm.statusReport.images.push({
